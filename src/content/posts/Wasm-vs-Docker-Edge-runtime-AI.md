...
To szczególnie istotne w kontekście twojej pracy z multi-agent systemami — Edge.js był projektowany specjalnie z myślą o uruchamianiu aplikacji JS, MCP-ów i agentów bez Dockera. Oznacza to, że serwer MCP może żyć na edge node'zie blisko użytkownika, z sub-milisekundowym cold startem.
...
Inżynierowie Wasmera przestali dotykać kodu bezpośrednio. Zamiast tego zaczęli *kierować* Codexem — wskazywali gdzie chcą dojść, a AI realizowała ścieżkę. To dokładnie ten shift, który opisujesz budując swój system dla Goldco — od operatora do orchestratora.
...
Jeśli budujesz SKILL.md marketplace z remote MCP execution (V4) — Edge.js to potencjalnie idealna warstwa hostingowa: zero Dockera, natywna kompatybilność z Node.js, izolacja sandboxem per-tenant z WebAssembly, sub-ms cold start. Warto śledzić jak Wasmer udostępni to na swojej platformie edge.
...
**Przykład praktyczny:** marketplace wtyczek (twój SKILL.md marketplace), gdzie różni klienci/tenanci uruchamiają własne skrypty/logikę — każdy w izolowanym sandboxie, bez potrzeby stawiania osobnych kontenerów per klient. Koszty spadają o rząd wielkości.
...
**Przykład:** Goldco.pl z systemem wtyczek dla partnerów (np. automatyczne wyliczanie spread/premium dla różnych klientów B2B) — każdy partner dostarcza własną logikę, która działa w sandboxie bez ryzyka dla reszty systemu.
...
**Przykład:** agent AI generuje i wykonuje skrypty analityczne na danych cenowych złota — bez ryzyka, że wygenerowany kod zmodyfikuje bazę danych lub wykona syscall poza scope'em.
...
**Przykład:** serwer MCP z logiką Trend Nawigatora — zamiast centralnego serwera, logika sygnałów działa na edge node blisko użytkownika, z latency <5ms zamiast 100-200ms przy roundtripie do Warszawy.
...
**Przykład:** Twój stack Python (analityka) + TypeScript (API) + Pine Script (logika sygnałów przetłumaczona do Wasm) — działające jako kompozytowe mikroserwisy bez kontenerów.
...
Najistotniejsze to kombinacja: **plugin marketplace z per-tenant sandboxingiem** (bezpośredni fit dla SKILL.md V4) + **edge hosting MCP-ów** bez kosztów infrastrukturalnych Dockera.

---

# Czytelna mapa — od pytania "czy to zamknięty ekosystem?" po konkretne ścieżki instalacji na swoim Hetznerze.
...
Żadna z tych opcji nie wymaga chmury Wasmera — wszystko działa na twoim Hetznerze jako zwykłe procesy lub systemd services.
...
- **Plugin marketplace** — twój przypadek ze SKILL.md
...
│  • SKILL.md plugins (per-tenant sandbox)        │
